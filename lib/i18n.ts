...
function detectSystemLanguage(): Language {
  ...
  // If browser language starts with zh (e.g., zh-CN, zh-TW), return Chinese
...
export function useLanguage(): [Language, (lang: Language) => void] {
  ...
  useEffect(() => {
    // Initialize by reading language setting
    const storedLanguage = readLanguageFromStorage();
    setLanguageState(storedLanguage);

    // Create an event listener to trigger when localStorage changes
  ...
  const setLanguage = (lang: Language) => {
    ...
    // Trigger a custom event to notify other components that the language has changed
